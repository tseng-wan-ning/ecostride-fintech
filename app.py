...
st.markdown("""
    ...
    /* 🎯 新增一個高優先級的強制深綠色 Class，用來徹底破解全局黑色覆蓋 */
    .force-dark-green-title {
        color: #2D4A22 !important;
    }
    </style>
...
# ==========================================
    ...
    # 🎯 這裡加上了獨立定義的特定高優先級類別 `force-dark-green-title`，成功擊碎並覆蓋最上面的黑色全局樣式！
    st.markdown("<h2 class='force-dark-green-title' style='color:#2D4A22 !important; font-size:32px; font-weight:800;'>相關研究成果 ── 彭博精算終端動態沙盤</h2>", unsafe_allow_html=True)
    ...
    with tab_res4:
        ...
        這是因為在智慧合約中引入了 <b>3.0% 實體綠能最低托底保價機制 (Floor Yield)</b>，成功切斷了氣候環境對保戶回饋的負面傳論，具備完備的抗風險防禦力。
        """, unsafe_allow_html=True)

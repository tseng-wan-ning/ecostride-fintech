...
# 隱藏 Streamlit 預設元素並注入 60-30-10 極簡美學 CSS + 側邊欄與手機內 App 變白高級樣式
st.markdown("""
    <style>
    ...
    /* 手機內建 App 導覽按鈕：完全消滅圈圈，點選或 Hover 整條變白 */
    ...
    with col_ui_right:
        ...
        with content_slot:
            if app_tab == "🌿 總覽 (Dashboard)":
                ...
                st.markdown(f"""<div style="background-color:#F5F7F4; border:1px solid #B7CEAD; padding:10px; border-radius:10px; font-size:11px; text-align:center; font-weight:600; color:#2D4A22;">{status_desc}</div>""", unsafe_allow_html=True)
...
# ==========================================
    ...
    with col_res_right:
        ...
        if run_sim:
            ...
            for percent_complete in range(1, 101, 4):
                ...
                <div style="display: flex; gap: 12px; margin-bottom: 15px;">
                    <div class="metric-card" style="border-top: 4px solid #83A474; flex: 1;"><div class="metric-value-green">{min(100.0, fake_ratio):.2f}%</div><div class="metric-label">全域共贏機率 (Win-Win Ratio)</div></div>
                    ...
                    <div class="metric-card" style="border-top: 4px solid #B7CEAD; flex: 1;"><div class="metric-value-blue">{fake_wacc:.2f}%</div><div class="metric-label">綠能開發商資金成本 (WACC)</div></div>
        ...
        <div style="display: flex; gap: 12px; margin-bottom: 15px;">
            ...
            <div class="metric-card" style="border-top: 4px solid #B7CEAD; flex: 1;"><div class="metric-value-blue">{base_wacc:.2f}%</div><div class="metric-label">綠能開發商資金成本 (WACC)</div></div>
    ...
    with tab_res1:
        ...
        fig_user.update_layout(template="plotly_white", height=350, margin=dict(l=40,r=40,t=10,b=10))
    ...
    with tab_res2:
        ...
        fig_ins.update_layout(template="plotly_white", height=320, margin=dict(l=40,r=40,t=10,b=10))
